...
function ToastItem({ toast }: { toast: Toast }) {
  ...
  const getToastStyles = () => {
    const baseStyles = "border-l-4 bg-white shadow-lg rounded-lg p-4"
...
/**
 * Toast hook for authentication feedback
 */
export function useAuthToast() {
  ...
  const showLoginSuccess = useCallback((userName?: string) => {
    showSuccess(
      'Welcome back!',
      userName ? `Hello ${userName}, you're now signed in.` : 'You have successfully signed in.'
    )
  }, [showSuccess])
  ...
  const showPasswordChangeRequired = useCallback(() => {
    showWarning(
